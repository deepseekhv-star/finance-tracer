...
def _render_transaction_frame(transaction_model,category_model):
    result = transaction_model.get_all_transactions()
    st.dataframe(result)
...
def _render_transaction_card(model: TransactionModel,category_model, item: dict):
    ...
    is_editing = st.session_state.get("editing_transaction") == str(item["_id"]) #Dạt
    # Format the header
    transaction_type = item.get('type','Unknown')
    amount = item.get('amount', 0)
    date = item.get('date', datetime.now())
    last_modified = item.get('last_modified', datetime.now())
            ...
            with st.form(f"edit_form_{transaction_id}"):
                ...
                with col1:
                    type_value = st.selectbox(
                        ...
                        index=config.TRANSACTION_TYPES.index(item["type"])
                    )
                    ...
                    date_value = st.date_input(
                        "Update at",
                        value=last_modified.date() if isinstance(last_modified, datetime) else datetime.now().date()
                    )   

                # Cột phải
                with col2:
                    categories = category_model.get_category_by_type(type_value)
                    category_options = [c["name"] for c in categories]

                    category_value = st.selectbox(
                        "Category",
                        category_options,
                        index=category_options.index(item["category"])
                    )
                ...
                with col_save:
                    if  st.form_submit_button("💾 Save", use_container_width=True):
                        updated = {
                            "type": type_value,
                            "amount": amount_value,
                            "last_modified": datetime.combine(date_value, datetime.now().time()),
                            "category": category_value,
                            "description": description_value
                        }

                        if model.update_transaction(item["_id"], **updated):
...
def _render_create_transaction_form(transaction_model: TransactionModel, category_model):
    ...
    with col2:
        if transaction_type:
            categories_result = category_model.get_category_by_type(transaction_type)
