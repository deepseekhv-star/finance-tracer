...
def render_dashboard(analyzer_model:FinanceAnalyzer,
    ...
    with col1:
        get_date_range_option = st.selectbox(
            "Select Date Range",
            list(get_date_range_options().keys()),
            index=3 # Default to "Last 30 Days"
        ) 

    date_range  =get_date_range_options()
    start_date,end_date = date_range[get_date_range_option]
...
def _render_metrics(analyzer_model, start_date, end_date):
    ...
    with col1:
        st.metric("💸 Total Expense", format_currency(total_expenses))
...
def _render_charts(analyzer_model:FinanceAnalyzer, #type Hint
                   visualizer_model:FinanceVisualizer,
                    start_date,
                     end_date):
    ...
    with col1:
        ...
        if not category_spending.empty:
            ...
            st.plotly_chart(fig, width='stretch') #embeded plotly chart into streamlit
        else:
